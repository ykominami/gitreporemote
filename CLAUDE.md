...
- **`remotedef.py` / `RemoteDef` + `RemoteDef.Item`** — `RemoteDef` holds a remote name and child `Item`s. `RemoteDef.to_dict()` returns `{name: {fetch: url, push: url}}`. `Item` is a tree node with `kind` ∈ `{fetch, push, other, root, _none}`: `other` = remote name node, `fetch`/`push` = URL leaf nodes, `_none` = parse fallback, `root` = synthetic root from `Item.create_root()`. `item.value` is empty when `kind == "other"`.
